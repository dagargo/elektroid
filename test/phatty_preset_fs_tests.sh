...
echo "Testing upload..."
$ecli phatty-preset-ul "$srcdir/res/Moog Little Phatty Preset.syx" $TEST_DEVICE:/$TEST_PRESET
[ $? -ne 0 ] && exitWithError 1
...
[ "$actual_cksum" != $(cksum "$srcdir/res/Moog Little Phatty New Name.syx" | awk '{print $1}') ] && exitWithError 1
