...
echo "Testing ls..."
files=$($ecli sds:16b-1c:ls $TEST_DEVICE:/)
[ $? -ne 0 ] && exit 1
echo "$files" | head
echo '[...]'
...
echo "Testing upload..."
$ecli sds:16b-1c:ul $srcdir/res/connectors/silence.wav $TEST_DEVICE:/1
[ $? -ne 0 ] && exit 1

# If renaming is not implemented, this will fail.
echo "Testing mv..."
$ecli sds:16b-1c:mv $TEST_DEVICE:/1 "Foo"
[ $? -ne 1 ] && exit 1

echo "Testing download..."
$ecli sds:16b-1c:download $TEST_DEVICE:/1
[ $? -ne 0 ] && exit 1
