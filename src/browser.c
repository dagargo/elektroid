...
browser_clear (struct browser *browser)
{
  ...
  gtk_editable_set_text (GTK_EDITABLE (browser->dir_entry),
  ...
  while (!item_iterator_next (&child_iter))
    {
      ...
      if (!loading || !preferences_get_boolean (PREF_KEY_SHOW_FOLDER_SIZES))
	break;
			       ...
			       const gchar *child_rel_dir, struct item *item)
{
  ...
  if (cached)
    ...
    {
      item->size = browser_sum_dir_size (browser, child_dir, exts);
      gint64 *size_val = g_new (gint64, 1);
      *size_val = item->size;
      g_hash_table_insert (browser->folder_size_cache, child_dir, size_val);
    }
}
