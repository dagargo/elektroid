...
elektroid_rx_sysex_runner (gpointer data)
{
  ...
  if (progress.sysex_transfer.active)
    {
      *err = backend_rx_sysex (&backend, &progress.sysex_transfer);
      if (!*err)
	{
...
static void
elektroid_rx_sysex_runner_end (gpointer data, gpointer func_value,
			       gboolean completed)
{
  gint err = *((gint *) func_value);
...
elektroid_rx_sysex ()
{
  progress_run (elektroid_rx_sysex_runner, PROGRESS_TYPE_UPDATE, NULL,
		_("Receiving SysEx"), "", TRUE,
		elektroid_rx_sysex_runner_end);
}
...
static void
elektroid_tx_sysex_common_end (gpointer data, gpointer func_value,
			       gboolean completed)
{
  GSList *filenames = data;
  gint *err = (gint *) func_value;
  ...
  if (res == GTK_RESPONSE_ACCEPT)
    {
      ...
      progress_run (func, PROGRESS_TYPE_UPDATE, filenames,
		    _("Sending SysEx"), "", TRUE,
		    elektroid_tx_sysex_common_end);
    }
}
...
static void
elektroid_preferences_cancel (GtkWidget *object, gpointer user_data)
{
...
elektroid_preferences_window_delete (GtkWidget *widget, GdkEvent *event,
				     gpointer data)
{
  elektroid_preferences_cancel (preferences_window_cancel_button, NULL);
  return TRUE;
}

static void
elektroid_preferences_save (GtkWidget *object, gpointer user_data)
{
  gboolean v;
  gint i, prev, post;
  GtkTreeIter iter;
...
static void
elektroid_delete_files_runner_end (gpointer data, gpointer func_value,
				   gboolean completed)
{
  struct browser *browser = data;
  ...
  if (BROWSER_IS_SYSTEM (browser) &&
  ...
  else
    {
      progress_run (elektroid_delete_files_runner, PROGRESS_TYPE_PULSE,
		    ...
		    elektroid_delete_files_runner_end);
    }
}

static void
elektroid_name_change_accept (gpointer source, const gchar *name,
			      gpointer data)
{
  gint err;
  gchar *new_path;
  ...
  if (browser->fs_ops->options & FS_OPTION_SLOT_STORAGE)
    ...
    {
      enum path_type type = backend_get_path_type (browser->backend);
      new_path = path_chain (type, browser->dir, name);
    }
...
static void
elektroid_name_change (GtkWidget *object, gpointer data)
{
  struct browser *browser = data;
  const gchar *ext;
  gint sel_len;
  ...
  name_edit_text (browser, _("Rename"),
		  browser->fs_ops->max_name_len, item.name, 0, sel_len,
		  FALSE, elektroid_name_change_accept,
		  browser_get_item_path (browser, &item));
}
...
static void
elektroid_add_dir_accept (gpointer source, const gchar *name, gpointer data)
{
  gint err;
  struct browser *browser = source;
  enum path_type type = backend_get_path_type (browser->backend);
  gchar *dir = path_chain (type, browser->dir, name);

  err = browser->fs_ops->mkdir (browser->backend, dir);
  if (err)
    {
...
{
  struct browser *browser = data;
  name_new_text (browser, _("Add Directory"), browser->fs_ops->max_name_len,
		 elektroid_add_dir_accept);
}
			    ...
			    gpointer data)
{
  ...
  if (event->keyval == GDK_KEY_Menu)
	...
	{
	  elektroid_name_change (NULL, browser);
	}
      return TRUE;
    }
...
static void
elektroid_set_device_runner_end (gpointer data, gpointer func_value,
				 gboolean completed)
{
  struct backend_device *be_sys_device = data;
  ...
  if (be_sys_device->type == BE_TYPE_SYSTEM)
  ...
  else
    {
      progress_run (elektroid_set_device_runner, PROGRESS_TYPE_PULSE,
		    ...
		    _("Connecting..."), TRUE,
		    elektroid_set_device_runner_end);
    }
}
...
elektroid_run (int argc, char *argv[])
{
  ...
  g_signal_connect (preferences_window_save_button, "clicked",
		    G_CALLBACK (elektroid_preferences_save), NULL);
  g_signal_connect (preferences_window_cancel_button, "clicked",
		    G_CALLBACK (elektroid_preferences_cancel), NULL);
