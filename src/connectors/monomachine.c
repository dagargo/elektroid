...
monomachine_wave_msg_new (guint id, struct idata *sample)
{
  ...
  frames = MIN (sample_info->frames,
		sample->content->len / sizeof (gint16));
  ...
  g_byte_array_append (tx_msg, (guint8 *) name,
			    ...
			    struct idata *sample)
{
  ...
  output = g_byte_array_sized_new (MONOMACHINE_WAVE_FRAMES *
				   sizeof (gint16));
