...
backend_get_separator (struct backend *backend)
{
  const gchar *sep;
  enum backend_type type = backend ? backend->type : BE_TYPE_SYSTEM;
  if (type == BE_TYPE_SYSTEM)
    {
...
gchar *
backend_chain_path (struct backend *backend, const gchar * parent,
		    const gchar * child)
{
  const gchar *sep = backend_get_separator (backend);
  return g_build_path (sep, parent, child, NULL);
}

gchar *
backend_translate_path (struct backend *backend, const gchar * input)
{
  gchar *output, *o;
  const gchar *i;
  const gchar *sep = backend_get_separator (backend);
...
gchar *
backend_filename_from_uri (struct backend *backend, gchar * uri)
{
  if (!backend || backend->type == BE_TYPE_SYSTEM)
    {
...
gchar *
backend_filename_to_uri (struct backend *backend, gchar * filename)
{
  if (!backend || backend->type == BE_TYPE_SYSTEM)
    {
